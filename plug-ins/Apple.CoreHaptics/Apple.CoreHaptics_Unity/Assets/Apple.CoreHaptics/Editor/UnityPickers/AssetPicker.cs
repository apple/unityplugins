	...
	public class AssetPicker : EditorWindow
	{
		...
		private void DisplayAsset(HierarchyEntry entry)
		{
			...
			using (new EditorGUILayout.HorizontalScope())
			{
                ...
                // Begin changes from source
				GUILayout.Label(AssetDatabase.GetCachedIcon(entry.Path), GUILayout.Width(20f), GUILayout.Height(20f));
				iconRect = GUILayoutUtility.GetLastRect();
                // End changes from source
            ...
            // Second condition added to show icons
			if (rect.Contains(Event.current.mousePosition) || iconRect.Contains(Event.current.mousePosition))
			{
				// click & doubleclick
				if (selectionCallback != null)
				{
