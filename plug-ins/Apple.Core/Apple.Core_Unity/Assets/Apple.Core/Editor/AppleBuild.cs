...
namespace Apple.Core
{
    ...
    {   
        ...
        private static string GenerateEmbedNativeLibraryShellScript(string projectRelativeNativeLibraryRoot)
        {
            ...
            + "        if [ -d \"$folder\" ]; then\n"
            + "            # Debug symbols first: the embed loop below breaks as soon as it embeds a library, and\n"
            + "            # 'Name.framework' sorts before 'Name.framework.dSYM', so a pass inside that loop never\n"
            + "            # sees them. Copy them where Xcode collects dSYMs for the archive; without this, uploading\n"
            + "            # to App Store Connect warns that the archive has no dSYM for each plug-in framework.\n"
            + "            if [ -n \"$DWARF_DSYM_FOLDER_PATH\" ]; then\n"
            + "                for dsym in \"$folder\"/*.dSYM; do\n"
            + "                    if [ -d \"$dsym\" ]; then\n"
