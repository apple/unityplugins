...
class UnityProject:
    ...
    def __init__(self) -> None:
        ...
        self.plugin_asset_path : Path = None
        self.supported_platforms : dict[str, str] = dict() # {PlatformString:LibraryPath, ...}
...
class NativeUnityPluginManager:
    ...
    def ProcessNativeUnityPlugin(self, plugin_path : Path) -> None:
        ...
        unity_plugins_paths = list(native_plugin.unity_project.path.joinpath("Assets").glob('**/Plugins'))
        if len(unity_plugins_paths) < 1:
            CTX.printer.InfoMessage(f"{plugin_id} contains no native libraries. Treating project as a non-native plug-in project.")
        else:
            if len(unity_plugins_paths) > 1:
                CTX.printer.WarningMessage(f"{plugin_id} has multiple 'Plugins' folders under /Assets. Using first found at {unity_plugins_paths[0]}")

            native_plugin.unity_project.plugin_asset_path = unity_plugins_paths[0]
        ...
        for platform, command in build_commands.items():
            CTX.printer.StatusMessageWithContext(f"Building {plugin_id} native libraries for platform: ", platform, f"\n{CTX.printer.Indent(1)}")
        ...
        for platform_path in native_plugin.unity_project.plugin_asset_path.iterdir():
            if platform_path.is_dir():
                if platform_path.name in unity_platform_name_table:
                    platform_name = unity_platform_name_table[platform_path.name]
                    native_plugin.unity_project.supported_platforms[platform_name] = platform_path
                    CTX.printer.MessageWithContext("Found supported Unity platform: ", platform_name, CTX.printer.Indent(2))
                    CTX.printer.MessageWithContext("Platform path: ", platform_path, CTX.printer.Indent(3))
                    ...
                    CTX.printer.WarningMessage(f"Unknown platform {platform_path.name} found in Plugins folder at {platform_path.parent}\n")

        if len(native_plugin.unity_project.supported_platforms) < 1:
            CTX.printer.WarningMessage(f"No supported platforms found in: {Printer.Decorate(f'{native_plugin.unity_project.plugin_asset_path}', CTX.printer.theme.context_color)}")
    ...
    def BuildTests(self) -> None:
        for plugin_id, native_plugin in self.native_unity_plugin_table.items():
            ...
            for curr_platform in  native_plugin.unity_project.supported_platforms.keys():
                curr_test_build_identifier = f"{plugin_id}_{native_plugin.unity_project.version}_{curr_platform}"
                ...
                CTX.printer.StatusMessage(f"Building {curr_platform} tests.", f"\n{CTX.printer.Indent(2)}")
    ...
    def ValidateProjectVersions(self):
        ...
        if unsupported_plugin_count > 0:
            ...
            if utility.BooleanPrompt(CTX.printer, "Would you like the script to attempt project upgrade?"):
                installed_unity_versions : list[str] = self.GetUnityInstallationList()
                ...
                if len(installed_unity_versions) != 0:
                    if len(installed_unity_versions) == 1:
                        ...
                        target_unity_version = utility.SelectionPrompt(CTX.printer, "Please select the version of Unity to use for upgrade:", self.GetUnityInstallationList())
